...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadIndicators = async () => {
      ...
      let updated = chartData.map(p => ({
        ...
        ...(activeIndicators.includes('RSI') ? {} : { rsi: undefined }),
        ...(activeIndicators.includes('MACD') ? {} : { macd: undefined, macdSignal: undefined, macdHist: undefined }),
      }))
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#0b0e11] text-[#d1d4dc] font-sans flex flex-col overflow-hidden pt-[60px]">
        ...
        {/* Toast Notification */}
        {toast && (
          <div className={`fixed top-20 left-1/2 -translate-x-1/2 z-50 px-6 py-4 rounded-xl shadow-2xl border animate-slide-in-top min-w-[300px] text-center ${toast.type === 'success'
              ...
              <div className="flex gap-2 mb-3 flex-wrap">
                ...
                <div className="flex gap-1 bg-[#1e222d] rounded p-1">
                  {['1D', '1M', '3M', '6M', '1Y', '5Y'].map(tf => (
                    <button
                      ...
                      className={`px-2 py-1 text-[10px] font-bold rounded transition-colors ${timeframe === tf
                ...
                <div className="flex gap-1 bg-[#1e222d] rounded p-1">
                  <button
                    ...
                    className={`px-3 py-1 text-[10px] font-bold rounded ${chartType === 'line' ? 'bg-[#2962ff] text-white' : 'text-[#787b86] hover:text-white'
                      }`}
                  >
                    LINE
                  </button>
                  <button
                    ...
                    className={`px-3 py-1 text-[10px] font-bold rounded ${chartType === 'candle' ? 'bg-[#2962ff] text-white' : 'text-[#787b86] hover:text-white'
                      }`}
                  >
                    CANDLE
                  </button>
  ...
  `}
                ...
                <button
                  ...
                  className={`px-3 py-1 text-[10px] font-bold rounded ${showVolume ? 'bg-[#089981] text-white' : 'bg-[#1e222d] text-[#787b86] hover:text-white'
                    }`}
                >
                  📈 VOLUME
                </button>
              ...
              <div className="w-full min-h-0 flex-1 flex flex-col">
                {loading && chartData.length === 0 ? (
                  ...
                  /* 3. Line Chart with Zoom/Pan (lightweight-charts) */
                  <div className={`w-full overflow-hidden transition-all duration-300 ${isFullscreen ? 'h-[calc(100vh-185px)]' : 'h-[28rem]'}`}>
                    <EChartLine
                      data={chartData}
                      showSMA={activeIndicators.includes('SMA')}
                      showEMA={activeIndicators.includes('EMA')}
                    />
                  </div>
                )}
              </div>
